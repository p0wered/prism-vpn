...
/** Белое свечение ключевых цифр — по правилу «glow-элементы белые» */
...
function PeriodSwitch({ value, onChange }: { value: 7 | 30; onChange: (v: 7 | 30) => void }) {
  return (
    <div className="flex rounded-full bg-surface-2 p-1" role="tablist" aria-label="Chart period">
      {([7, 30] as const).map((p) => (
        <button
          key={p}
          type="button"
          ...
          className={`cursor-pointer rounded-full px-3.5 py-1.5 font-mono text-xs transition-colors ${
            value === p ? 'bg-white/12 text-fg' : 'text-fg-muted hover:text-fg'
          }`}
        >
          {p}d
        </button>
...
export function OverviewPage() {
  const { nickname, email, balance, daysLeft, expiresAt, devices, subToken } = useDashboard()
  ...
  return (
    <div className="flex flex-col gap-6">
      <Reveal>
        <PageHeader title="Overview" sub={`Welcome back, ${nickname} · ${email}`} />
      </Reveal>
        ...
        <Reveal delay={0.2}>
          <GlassCard className="flex h-full flex-col p-5 lg:p-6">
            ...
            <Link
              ...
              className="mt-3 inline-flex items-center gap-1.5 pl-2 text-xs text-fg-muted transition-colors hover:text-fg"
            >
